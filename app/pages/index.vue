...
function selectPin(type: EntityType, entity: Entity) {
  ...
  if (limit === 1) {
    ...
    pinItems(
      type,
      pinned[type].filter((s) => s.entity.id !== entity.id),
    );
  } else {
    // entity ยังไม่ได้ pin — ตรวจว่าอยู่ใน suggestion แล้วหรือยัง
    const inSuggestion = suggestion.value[type].find(
      (s) => s.entity.id === entity.id,
    );
    ...
    if (inSuggestion) {
      ...
      pinItems(type, [
        ...pinned[type],
        { entity, quantity: inSuggestion.quantity },
      ]);
...
function adjustQty(type: EntityType, entityId: number, delta: number) {
  ...
  if (next < 1) {
    ...
    pinItems(
      type,
      pinned[type].filter((s) => s.entity.id !== entityId),
    );
              ...
              >
                ...
                <td class="td-name">
                  ...
                  <template v-else-if="suggestion[type].length > 0">
                    <div
                      v-for="(item, i) in suggestion[type]"
                      :key="item.entity.id"
                      class="slot-item-row"
                    >
                      {{ item.entity.name }}
                ...
                <td class="td-status">
                  ...
                  <span v-else-if="pinned[type].length > 0" class="badge pin"
                    >ล็อก</span
                  >
                  <span
                    v-else-if="suggestion[type].length > 0"
                    class="badge auto"
                    >อัตโนมัติ</span
                  >
                </td>
                <td class="td-action">
                  <button
                    class="btn-sm"
                    :class="expandedSlot === type ? 'btn-active' : ''"
                    @click="toggleExpand(type)"
                  >
                    เลือก
                  </button>
                  <button
                    v-if="pinned[type].length > 0"
                    class="btn-sm btn-ghost"
                    @click="pin(type, null)"
                  >
                    ยกเลิก
                  ...
                  <div class="picker">
                    ...
                    <div class="picker-opts">
                      ...
                      <div
                        ...
                        :class="{
                          'pick-active':
                            pinned[type].length === 0 && !excluded[type],
                        }"
                      ...
                      <div
                        ...
                        :class="{ 'pick-active': excluded[type] }"
                      ...
                      >
                        ...
                        <div class="pick-right">
                          <span class="pick-price">{{
                            fmt(slotTotal([{ entity, quantity: 1 }]))
                          }}</span>
                          <template
                            v-if="
                              pinned[type].some(
                          ...
                          >
                            <div class="qty-row">
                              <button
                                class="qty-btn"
                                @click="adjustQty(type, entity.id, -1)"
                              >
                                −
                              </button>
                              <span class="qty-num">{{
        ...
        <div class="sum-box">
          ...
          <template v-if="budget !== null">
            ...
            <div class="sum-row">
              <span>คงเหลือ</span>
              <span
                :class="(budgetRemaining ?? 0) < 0 ? 'text-err' : 'text-ok'"
              >
        ...
        <div v-if="issues.length" class="sum-box">
          ...
          >
            ...
            <div>
              ...
              <div v-if="issue.resolution" class="text-muted">
                {{ issue.resolution }}
              </div>
            </div>
          </div>
...
.pick-row-exclude:hover {
  background: #fff0f0;
}
.pick-row-exclude.pick-active {
  background: #ffe8e8;
  border-color: #cc3333;
}
.pick-divider {
